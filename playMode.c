	...
	SDL_AudioDeviceID* audioDevice, SDL_AudioSpec* wavSpec, unsigned short size)
{
	...
	if (Sound_Land == NULL)
		Sound_Land = loadSound("land.wav", wavSpec);
	if (Sound_Rotate == NULL)
		Sound_Rotate = loadSound("rotate.wav", wavSpec);
	if (Sound_Complete == NULL)
		Sound_Complete = loadSound("complete.wav", wavSpec);
	if (Sound_Over == NULL)
		Sound_Over = loadSound("over.wav", wavSpec);
	...
	if (*inputLock == false && inputLockPressed() && !*gameOver)
	{
		...
		if (currentPiece != NULL)
		{
			...
			if (GetAsyncKeyState(VK_LEFT) && !isColliding(*currentPiece, *X, Y, LEFT, mapData))
			{
			...
			else if (GetAsyncKeyState('X') && *X + currentPiece->height <= WIDTH_OF_PLAYSPACE &&
				*Y + currentPiece->width <= HEIGHT_IN_CHARS - 2)
			{
